...
<script>
			...
			success   : function(data) {
				...
				if(data.jsonObj.listSize != 0){
					...
					for(var i=0; i < data.jsonObj.list.length; i++){
						...
						jobTitleListStr += "<tr>";
						jobTitleListStr += "	<td class=\"headline\"><a href=\"/home/aboutUs/jobtitle_view.do?lang=vi&sid=" + data.jsonObj.list[i].id + "\" onclick=\"jobTitleDetailView('"+data.jsonObj.list[i].id+"');return false;\" style=\"cursor: pointer;\">" + data.jsonObj.list[i].title + "</a></td>"; // Title
						jobTitleListStr += "	<td>" + data.jsonObj.list[i].location + "</td>"; // Location
						jobTitleListStr += "	<td>" + date + "</td>"; // Time
						jobTitleListStr += "</tr>";
					}
				}else{
					jobTitleListStr += "<tr><td class=\"no_result\" colspan=\"3\">Không tìm thấy dữ liệu</td></tr>";
			 	}
				// 리스트 세팅
	...
	function jobTitleDetailView(sid) {
		...
		location.href="/home/aboutUs/jobtitle_view.do?lang=vi&sid=" + sid + "&from=" + $("#mvStartDate").val() + "&to=" + $("#mvEndDate").val() + "&page=" + page;
	}
</script>
<!--container : 서브페이지 컨텐츠 -->
            ...
            <div class="search">
                <form action="">
                    <fieldset class="search_wrap">
                        ...
                        <div class="keyword">
                            <input type="button" value="Tìm kiếm" class="btn_search" onclick="getAllJobTitleList();"/>
                        </div>
                    </fieldset>
                </form>
            </div>
