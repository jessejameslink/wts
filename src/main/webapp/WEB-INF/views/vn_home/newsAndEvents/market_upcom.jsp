...
<script>
	$(document).ready(function() {
		...
		if("${type}" == null || "${type}" == "") {
			...
			searchMarketUpcomNewsList("${schSkey}");
		}
	});

	function searchMarketUpcomNewsList(str) {
		$("#grdMarketUpcomNews").find("tr").remove();
		$("#newsNext").val("");
		$("#newsSkey").val("");
		$("#newsSkeyNxt").val("");
		$("#newsIdx").val("0");
		getMarketUpcomNewsList(str);
	}
	...
	function getPreList() {
		...
		if(newsSkey.length > 0) {
			getMarketUpcomNewsList(newsSkey[newsIdx - 1]);
		} else {
			getMarketUpcomNewsList("");
		}
	}
	...
	function getNextList() {
		...
		$("#newsIdx").val(newsIdx);
		if($("#newsNext").val() == "Y") {
			getMarketUpcomNewsList($("#newsSkeyNxt").val());
		}
	}

	function getMarketUpcomNewsList(skey) {
				...
				if(data.stockNewsList != null) {
					...
					if(data.stockNewsList.list1 != null && data.stockNewsList.list1.length > 0) {
						for(var i=0; i < data.stockNewsList.list1.length; i++) {
							...
							htmlStr += "	<td class=\"headline\"><a href=\"/home/newsAndEvents/market_view.do?lang=vi&sid=" + stockNewsList.seqn + "\" onclick=\"miraeUpcomView('"+stockNewsList.seqn+"');return false;\" style=\"cursor: pointer;\">" + stockNewsList.titl + "</a></td>"; // Title
							htmlStr += "</tr>";
						}
						$("#grdMarketUpcomNews").html(htmlStr);
					} else {
						htmlStr += "<tr>";
...
<div id="container" class="sub">
	<form id="frmMiraeView" action="/home/newsAndEvents/market_view.do" method="post">
		<input type="hidden" id="typeS" name="type" value="Upc"/>
        ...
        <div id="contents">
            ...
            <div class="tab">
                <div>
                    ...
                    <a style="cursor: pointer;" onclick="marketGo('/home/newsAndEvents/market_upcom.do')" class="on">UPCOM</a>
                ...
                <form action="">
                    <fieldset class="search_wrap">
                        ...
                        <div class="keyword">
                            ...
                            <input type="button" value="Tìm kiếm" class="btn_search" onclick="searchMarketUpcomNewsList()"/>
                        </div>

                        <div class="date">
                            <span class="date_box">
                                <input id="searchKey" type="text" id="fromDate" name="fromDate" title="search start date" class="datepicker"/>
