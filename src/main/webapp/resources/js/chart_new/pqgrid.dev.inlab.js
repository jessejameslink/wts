 ...
 * 2015-07-14, 변만복, 웹 접근성 추가

 * 2015-07-14, 변만복, nfoc_flag 처리 확인 필요(wa.js 에 동작 되어 있음)

 * 2015-07-14, 변만복, 포커스 이동 및 title 처리 확인 필요
...
(function($) {
	...
	fnSB.decr_cur_pos = function(evt) {
		...
		if (steps) {
			...
			// cur_pos가 0일 경우에도 스크롤 이벤트 발생하도록 변경
...
(function($) {
	...
	_pGenerateView.generateView = function(objP) {
		...
		var self = this,
			...
			DM = o.dataModel, // 2016-04-08 김행선, 조회된~ 메세지 처리
		...
		if (objP) {
			...
			if (that.totalVisibleRows === 0) {
				...
				if(DM.noDataMsg !==undefined && DM.noDataMsg != null && DM.noDataMsg) {
	...
	_pGenerateView.generateView2 = function(objP) {
			...
			if (pqpanes.h && pqpanes.v) {
				...
				if (pqpanes.v) {
					...
					if (pqpanes.h) {
						...
						var $tbl = $div_child.children("table");

						var tblHt = $tbl[0].scrollHeight + 7;
						...
						if(tblHt<contHt) {
							...
							$div_child.css({
								...
								maxHeight: contHt //천창환 height -> maxHeight
							...
							$div_child.css({
								...
								height: contHt //천창환 height -> maxHeight
	...
	_pGenerateView._generateTables = function(initV, finalV, objP) {
		...
		if (1 === 1) {
			...
			if(typeof o.caption != 'undefined' && o.caption != '') {

				caption = o.caption;

			} else {

				if(typeof o.titleColumns != 'undefined' && o.titleColumns != null && o.titleColumns.length > 0) {

					for(var col = 0; col < o.titleColumns.length; col++) {

						caption += o.titleColumns[col].title + ',';

					}

					caption = o.title + ':' + caption.substr(0, caption.length-1) + '(으)로 구성된 표';
			...
			buffer.push("<caption>" + caption + "</caption>");
			...
			if (numberCell.show) {
				...
				buffer.push("<th scope='col' style='width:" + wd + "px;' ></th>");
			...
			for (var col = 0; col <= finalH; col++) {
				...
				buffer.push("<th scope='col' style='width:", wd, "px; height: 0px' pq-col-indx='", col, "'>" + column.title + "</th>");
	...
	_pGenerateView.renderCell = function(objP) {
		...
		// 2015-07-14, 변만복, 확인 필요, title 등등 생성 되는지 확인 필요, 없을경우 title 처리 필요

		var stock = '';

		if(o.stockCode != null && o.stockName != null) {

			stock = "[" + o.stockCode + "] " + o.stockName;
		...
		*/
		...
		// 웹접근성 그리드 포커스 인 초점 이동처리

		var tabIndex = '';

		if(o.selectionModel.type != 'row') {
		...
		// var str = ["<td ", tabIndex," class='", cls, "' ", attr, stock != '' ? "title='" + stock + "'" : '', " >", this._renderCell(objP, dataCell), "</td>"].join("");

		var str = ["<td ", tabIndex, " class='", cls, "' ", attr, " >", this._renderCell(objP, dataCell), "</td>"].join("");
	...
	_pGenerateView._generateRow = function(rowData, rowIndx, CM, buffer, objP, lastFrozenRow, nestedshow) {
		...
		if (rowData.pq_rowselect) {
			...
			row_cls += " pq-row-select ui-state-highlight nfoc_flag";

			selectedTitle = " title='선택'";
		...
		// 2015-07-14, 변만복, 확인 필요, title 등등 생성 되는지 확인 필요, 생성되면 제거 필요(for문 continue)
		...
		// buffer.push("<tr ", tabIndex, " pq-row-indx='", rowIndx, "' class='", row_cls, "'", selectedTitle, ">");

		buffer.push("<tr ", tabIndex, " pq-row-indx='", rowIndx, "' class='", row_cls, "'", selectedTitle, " ", attr, ">");
...
*/
		...
		sortable: false, // true,				// 정렬 default: false 로 변경, 정렬이 필요한 경우 각 그리드에서 sortable: true로 옵션 주기
		...
		colResizable: true,						// 2014-09-18 변만복, columnResizable 처리 추가(default: false - column크기조절 X)

		noRowsTextView: true,

		refreshViewTimeout: null,
	...
	fn._create = function() {
		...
		//this.setEditorPosTimer(); 천창환 2016.04.04
	...
	fn._saveDims = function() {
		...
		/*var $cont = this.$cont, $tblb = this.$tbl, round= Math.round;

		if ($tblb) {

			for (var i = 0, len = $tblb.length; i < len; i++) {
				...
				$tbl.data("offsetHeight", round(tbl.offsetHeight));

				$tbl.data("scrollWidth", round(tbl.scrollWidth))
		...
		if ($tblh) {
			...
			for (var i = 0, len = $tblh.length; i < len; i++) {

				var tbl = $tblh[i],

					$tblParent = $(tbl).parent();

				$tblParent.data("offsetHeight", round(tbl.offsetHeight));

				$tblParent.data("scrollWidth", round(tbl.scrollWidth))
	...
	fn._onMouseWheel = function(evt) {
		...
		if ((horizontal && o.virtualX) || (!horizontal && o.virtualY)) {
			...
			// 20160331 김행선 스크롤링 수정

			if(new_pos == -1 && cur_pos == 0){
	...
	fn.sRefresh = function(objP) {

		this.iRefresh._sRefresh(objP)
	...
	fn._addRowsData = function(obj) {
		...
		var DM = this.options
		var maxRows = DM.maxRows || undefined;

		try {

			//천창환 2016.04.08

			this.options.dataModel.noDataMsg=true;
			...
			if (rowIndx == null) {
				...
				if(maxRows !=undefined && maxRows!=null) {
					if(data.length>maxRows){
						for(var y=data.length; maxRows < y ; y--) {
							data.splice(y-1, 1);
						}
					}
				}
				...
				if (rowIndx < data.length || data.length == 0) {
	...
	fn.addRows = function(obj) {
		...
		if (this._addRowsData(obj)) {
			...
			var sRefresh = obj.sRefresh || false; //2015.07.17 천창환
	...
	fn._digestData = function(ui) {
		...
		var that = this,
			...
			checkEditable = (checkEditable == null) ? false : checkEditable,

			//checkEditable = (checkEditable == null) ? true : checkEditable, 천창환 2016.03.04
		...
		for (var i = 0; i < rowListLen; i++) {
			...
			if (type == "add") {
				...
				for (var j = 0, lenj = CM.length; j < lenj; j++) {
					...
					var column = CM[j],	dataIndx = column.dataIndx, dataIndx2 = column.dataIndx;



					//천창환 2015.09.14

					//if(typeof newRow[dataIndx2]=='undefined') {

					//	dataIndx2=dataIndx.substring(3);

					//}

					

					newRow[dataIndx] = newRow[dataIndx2]
				...
				if (type == "add") {
					...
					if(maxRows !=undefined && maxRows!=null) {
						...
						if(data.length>maxRows){
							...
							for(var k = maxRows-1, y=data.length; k < y ; k++, y--) {
			...
			if (evt.keyCode == EM.saveKey) {
				...
				if (evt.keyCode == keyCodes.ESCAPE) {
					...
					if (SM.type == "cell") {
						...
						// $(that.$tbl).find('td[class^=pq-grid-cell]').removeAttr("tabindex");
						...
						if (SM.type == "row") {
							...
							// $(that.$tbl).find('tr[class^=pq-grid-row]').removeAttr("tabindex");
										...
										if (keyCode == keyCodes.ENTER) {
											...
											if (SM.type == "row") {
												...
												if (rowLastSel != null) {
													...
													// $($(that.$tbl)[0].rows[that.iRows.lastSelection.rowIndx+1]).trigger('rowDblClick');

													// $($(that.$tbl)[0].rows[that.iRows.lastSelection.rowIndx+1]).trigger('rowClick');

													

													$(that.$tbl).trigger('pqgridrowdblclick', that.iRows.lastSelection);

													$(that.$tbl).trigger('pqgridrowclick', that.iRows.lastSelection);
	...
	fn._calcThisColModel = function() {
		...
		// 2014-09-18 변만복, columnResizable 처리 추가(default: false - column크기조절 X)

		if(!o.colResizable) {

			for (var i = 0, len = CMLength; i < len; i++) {

				CM[i].resizable = false;
...
(function($) {
	...
	_pRefresh._setScrollVNumEles = function() {
		...
		var tbl, $tbl, htTbl = 0, $that_tbl=that.$tbl;

		if ($that_tbl && $that_tbl.length > 0) {

			tbl = $that_tbl[$that_tbl.length - 1];
	...
	_pRefresh.setContAndHeaderHeight = function() {
		...
		if (options.height !== "flex") {
			...
			var ht = (this.height - that.$header_o.height() - ((options.showTop) ? that.$top[0].offsetHeight : 0) - that.$bottom[0].offsetHeight);
	...
	_pRefresh.refreshGridWidthAndHeight = function() {
		...
		if (widthPercent || heightPercent) {
			...
			if (wdParent == 0) {
				var _tagName=parent[0].tagName.toUpperCase();
				while (_tagName != "BODY") {
	...
	_pRefresh.init = function() {
		...
		//this.rowHt = 22 + (o.rowBorders ? 1 : 0); 천창환 2016.03.08 row 디폴트 높이 수정

		this.rowHt = 18 + (o.rowBorders ? 1 : 0);
	...
	_pRefresh._sRefresh = function(objP) {
		...
		*/
		...
		//if (table !== false) {
			...
			that.iGenerateView.generateView2()
...
(function($) {
	...
	_pGenerateView._generateDetailRow = function(rowData, rowIndx, thisColModel, buffer, objP, lastFrozenRow) {
		...
		buffer.push("<tr pq-row-indx='" + rowIndx + "' class='" + row_cls + "'", selectedTitle, ">");
	...
	_pSort.sortLocalData = function(data) {
		...
		var that = this.that,
			...
			viewName = that.options.viewName, 

			CM = that.colModel,

			sorters = this.sorters,

			isInter = false; // 관심종목인지
		...
		// 2016.05.03 김행선, 0169,0170에서 소팅시, 공백이나, 빈데이터 제외하도록 수정함

		if(viewName == "0169" || viewName == "0170"){

			var data = this._sortLocalData(sorters, data);

			var data2 = data.slice(0);

			for(var i=0,dataCnt=0,len=data2.length;i<len;i++,dataCnt++){

				var hasData = false;

				for(var j=0,len2=data2[i].length;j<len;j++){

					if(data2[i][j] !== undefined){
				...
				if(!hasData) {
					...
					data.push(data.splice(dataCnt,1)[0]);
	...
	_pSort._sortLocalData = function(sorters, data) {
		...
		function sort_integer(obj1, obj2, dataIndx, dir) {
			...
			val1 = val1 == "" ? -1 : val1 ? parseInt(val1, 10) : 0;

			val2 = val2 == "" ? -1 : val2 ? parseInt(val2, 10) : 0;
		...
		function sort_string(obj1, obj2, dataIndx, dir) {
			...
			if(val1 == "") return -1 * dir

			if(val2 == "") return 1 * dir
			...
			if(val1 == "0"){
				...
				// 2016.05.03 김행선, +,- 구분값 있는 데이터 정렬 처리

				var gubn = val1.trim().substring(0,1);

				if(gubn == "-" || gubn == "+"){

					val1 = Number(val1.trim().rmcomma().substring(1));
			...
			if(val2 == "0"){
				...
				gubn = val2.trim().substring(0,1);

				if(gubn == "-" || gubn == "+"){

					val2 = Number(val2.trim().rmcomma().substring(1));
	...
	fn._onDataAvailable = function(objP) {
		...
		//if (FM && FM.on && ((location == "local" && FM.type != "remote") || (location == "remote" && FM.type == "local"))) { 천창환 2016.04.05
...
(function($) {
	...
	fnGrid.addRow = function(objP) {
		...
		var that = this,
			...
			sRefresh = objP.sRefresh || false, //2015.07.17 천창환
...

		{
			...
			}else if(o.ieVersion=='8'){

				msTime=1000;

			}else if(o.ieVersion=='9'){

				msTime=800;

			}else if(o.ieVersion=='10'){

				msTime=600;

			}else if(o.ieVersion=='11'){
		...
		if (objP.refresh !== false) {
			...
			if(sRefresh) { //2015.07.17 천창환

				

				o.refreshViewCount++;

				if(o.refreshViewTimeout!=null){

					if(o.refreshViewCount>5){

						clearTimeout(o.refreshViewTimeout);

						o.refreshViewTimeout=null;

						setTimeout(function(){

							o.refreshViewCount=0;
				...
				if(o.refreshViewTimeout==null){
					...
					o.refreshViewTimeout=setTimeout(function(){
...
(function($) {
	...
	_pR._boundRow = function(obj) {
		...
		$tr.addClass("pq-row-select ui-state-highlight nfoc_flag");
		...
		if (isSelected) {
			...
			if (this.inViewRow(rowIndxPage)) {
				...
				if ($tr) {
					...
					$tr.removeClass("pq-row-select ui-state-highlight nfoc_flag");
