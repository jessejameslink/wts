  ...
  nexUtils.prototype = {
    ...
    /**
     * 대우기간계 TR 숫자 형식 변환 시작
     * date : 2016.10.21
     * author : 남향선 (홈페이지쪽 수정)
     */

    /**
     * numberFormatDw에서 value가 null일때 undefined떠서 추가
     **/
    numberFormatWN: function(value, format){
    	var result = this.numberFormatDw(value, format);
    	if(result==""){
    		if(format=="###,###"){
    			return 0;
    		}else{
     ...
     **/
    numberFormatWNV2: function(value, format, nullDefault){
    	var result = this.numberFormatDw(value, format);
    	if(result==""){
    		if(!this.isCommonNull(nullDefault)){
    			return nullDefault;
    		}
    		if(format=="###,###"){
    			return 0;
     ...
     * 			[format]- 변환 포맷
     */
    numberFormatDw: function(num, format,nonSign){
    	...
    	num = num.split(".");
    	//소수점 이상 변환
    	var tmp_fm = this.replaceAllDw(format[0], ",", "");
    	if(tmp_fm.length > num[0].length){
    		if(this.startWith(tmp_fm, "0")){
    			num[0] = this.fillLeft(num[0], tmp_fm.length, "0");
    		}
    	}
    	//콤마(,)가 있을 경우 콤마 삽입
    	...
    	if(format.length > 1){
    		if(num.length > 1){
    			if(format[1].length > num[1].length){
    				num[1] = this.fillRight(num[1], format[1].length, "0");
    			}else{
    				num[1] = num[1].substr(0, format[1].length);
    			}
    		}else{
    			num[1] = this.fillRight("", format[1].length, "0");
    		}
    	}else{
    		if(num.length > 1){
    			num.pop();
    ...
    /**
     * 메소드명 : 문자열 치환
     * 사용방법 : replaceAllDw("abcada", "a", "1") => "1bc1d1"
     * 파라미터 :	[str](필수)- 치환 대상 문자열
     * 			[org](필수)- 치환대상 문자
     * 			[rep](필수)- 치환문자
     */
    replaceAllDw: function(str, org, rep){
     ...
     * 			[chr](필수)- 채울 문자
     */
     fillRight: function(ori, size, chr){
    	//길이 체크
    	if(ori.length >= size){
    		return ori;
    	}
     ...
     * 			[chr](필수)- 채울 문자
     */
    fillLeft: function(ori, size, chr){
  ...
  /////////////////////////////// [DATE TIME FORMAT START] ///////////////////////////////
    ...
    dateFormat: function(date, format) {
      ...
      if (date.length == 8) { // yyyyMMdd
		  ...
		  arr_data[0]=date.substring(0, 4);

		  arr_data[1]=date.substring(4, 6);

		  arr_data[2]=date.substring(6);
        ...
        //result = date.substring(0, 4) + format + date.substring(4, 6) + format + date.substring(6);
		  ...
		  arr_data[1]=date.substring(4);

        //result = date.substring(0, 4) + format + date.substring(4);

      } else if (date.length == 4) { // MMdd

		  arr_data[0]=date.substring(0, 2);

		  arr_data[1]=date.substring(2);
    ...
    addDate: function(pInterval, pAddVal, pYyyymmdd, pDelimiter) {
      ...
      if (pDelimiter !== undefined && pDelimiter != '' ) {
        ...
        pYyyymmdd = String(pYyyymmdd).replaceAll(pDelimiter, '');      
      ...
      yyyy = String(pYyyymmdd).substring(0, 4);

      mm = String(pYyyymmdd).substring(4, 6);

      dd = String(pYyyymmdd).substring(6, 8);
      ...
      } else if (pInterval == 'mm') {

        mm = (mm * 1) + (pAddVal * 1);

      } else if (pInterval == 'dd') {
	...
	**/
	...
	numberFormat:function(num, cipher){
		...
		if(String(num).indexOf('.')>-1) {
			...
			arr_num[0]=arr_num[0].commify();

			arr_num[1]=arr_num[1].rPad(cipher,'0');
			...
			arr_num[0]=String(num).commify();

			arr_num[1]=String('').rPad(cipher,'0');
...
jQuery.fn.numbericOnly = function () {
