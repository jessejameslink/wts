 ...
 * @author 김대원
 */
public class Pool implements IConstants{
	...
	private void init(){
		...
		this.makePoolTime = System.currentTimeMillis();
		...
		for(int current = 0; current<size; current++){
			...
			if(conn==null){
				try{
					...
					//System.out.println("[POOL][connection is null]["+id+"]["+hostInfo[id].getHost()+"]["+hostInfo[id].getPort()+"]");
	...
	public static void main(String[] args){
		try{
			...
			for(int i=0;i<host.length;i++){
				//@TODO: delete
				//host[i] = "172.16.100.86";
				//host[i] = "10.0.31.9";
				host[i]	=	SystemConfig.getBPHost();
				//System.out.println(host[i]);
			}
			Pool.makePool(host, 15204);
		}catch(Exception e){
			e.printStackTrace();
		}
	}
