...
<script>
			...
			success   : function(data) {
				...
				if(data.jsonObj.listSize != 0){
					...
					for(var i=0; i < data.jsonObj.list.length; i++){
						...
						miraeNewsListStr += "	<td class=\"headline\"><a href=\"/home/question/question_view.do?lang=vi&sid=" + data.jsonObj.list[i].id + "\" onclick=\"miraeNewsDetailView('"+data.jsonObj.list[i].id+"');return false;\" style=\"cursor: pointer;\">" + data.jsonObj.list[i].title + "</a></td>"; // Title
						miraeNewsListStr += "</tr>";
					}
				}else{
	...
	function miraeNewsDetailView(sid) {
		location.href="/home/support/question_view.do?lang=en&sid=" + sid + "&from=" + $("#mvStartDate").val() + "&to=" + $("#mvEndDate").val() + "&key=" + $("#searchKey").val() + "&page=" + page;
	}
